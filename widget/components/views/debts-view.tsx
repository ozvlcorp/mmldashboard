...
export function DebtsView({
  ...
  // На демо-данных kind не проставлен → считаем должниками; иначе фильтр по знаку.
  const byKind = useMemo(
    () =>
      allRows.filter((d) =>
        d.kind ? d.kind === (tab === 'debtors' ? 'debtor' : 'creditor') : tab === 'debtors',
      ),
    [allRows, tab],
  );
  const debtors = useMemo(
    () => (groupId ? byKind.filter((d) => d.groupId === groupId) : byKind),
    [byKind, groupId],
  );

  const totalDebt = debtors.reduce((s, d) => s + d.debtAmount, 0);
  const avgDebt = debtors.length > 0 ? totalDebt / debtors.length : 0;
  const debtorsCount = allRows.filter((d) => d.kind !== 'creditor').length;
  const creditorsCount = allRows.filter((d) => d.kind === 'creditor').length;
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <div className="inline-flex rounded-lg border border-(--color-border) bg-(--color-card) p-0.5">
              <button
                ...
                className={
                  'h-7 rounded-md px-3 text-[12px] font-semibold transition-all ' +
                  (tab === 'debtors'
                    ? 'bg-(--color-danger) text-white shadow-[0_2px_6px_rgba(239,68,68,0.25)]'
                    : 'text-(--color-fg-soft) hover:text-(--color-fg)')
                }
              >
                Должники
                <span className="ml-1.5 opacity-80">{debtorsCount}</span>
              </button>
              <button
                type="button"
                ...
                className={
                  'h-7 rounded-md px-3 text-[12px] font-semibold transition-all ' +
                  (tab === 'creditors'
                    ? 'bg-(--color-warning) text-white shadow-[0_2px_6px_rgba(245,158,11,0.25)]'
                    : 'text-(--color-fg-soft) hover:text-(--color-fg)')
                }
              >
                Кредиторы
                <span className="ml-1.5 opacity-80">{creditorsCount}</span>
              </button>
            </div>

            {groups.length > 0 && (
              <label className="inline-flex items-center gap-1.5 text-[12px]">
                <span className="text-(--color-muted-fg)">Группа:</span>
                <select
                  value={groupId}
                  onChange={(e) => setGroupId(e.target.value)}
                  className="h-7 rounded-md border border-(--color-border) bg-(--color-card) px-2 text-[12px] focus:border-(--color-primary)/40 focus:outline-none focus:ring-2 focus:ring-(--color-primary)/20"
                >
                  <option value="">Все ({byKind.length})</option>
                  {groups.map((g) => {
                    const n = byKind.filter((d) => d.groupId === g.id).length;
                    return (
            ...
            {groupId && (
              <button
                type="button"
                onClick={() => setGroupId('')}
                className="rounded-md bg-(--color-primary-soft) px-2 py-0.5 text-[11px] font-semibold text-(--color-primary-soft-fg) hover:bg-(--color-primary-soft)/70"
              >
                Сбросить фильтр
              </button>
            )}
        ...
        <CardContent className="px-0 py-0">
          {debtors.length === 0 && !scanning ? (
            <div className="px-6 py-10 text-center text-[13px] text-(--color-muted-fg)">
              {onScan
                ? tab === 'debtors'
                  ? 'Нет контрагентов с отрицательным балансом (вам никто не должен).'
                  : 'Нет контрагентов с положительным балансом (вы никому не должны).'
                : 'Нет данных.'}
            </div>
          ) : (
            <DataTable
              key={`debts-${tab}-${groupId}`}
              data={debtors}
              columns={columns}
              rowKey={(d) => d.demandId}
              defaultSort={{ key: 'debt', dir: 'desc' }}
              tableId="debts"
              exportName={`OY_Analytics_${tab === 'debtors' ? 'Debtors' : 'Creditors'}`}
            />
          )}
        </CardContent>
      </Card>
